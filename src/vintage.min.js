...
3E3,function(){$(this).fadeOut(300,function(){$(this).remove()})})};(function(){var g=f.position();g.top+=Math.round(f.height()/2);g.left+=Math.round(f.width()/2);i=jQuery('<div class="vintage-loader">Loading&hellip;</div>');i.css("top",g.top+"px").css("left",g.left+"px").hide().appendTo("body").fadeTo(0,0.8,function(){p()})})()}})};